---
title: App Builder First App related pages
description: Links to related pages in App Builder First App section
---
# App Builder First App related links

## App Builder - Build your First App related pages

* [Overview](overview.md)
* [The `.env` file](env-file.md)
* [The `app.config.yaml` file](app-config-yaml-file.md)
* [`actions` folder](actions-folder.md)
* [`test` folder](test-folder.md)
* [`web-src` folder](web-src-folder.md)
* [Testing Locally](testing-locally.md)
* [Testing Using App Builder URL](testing-app-builder-url.md)
* [Conclusion](conclusion.md)
* [Sample Module source code](https://github.com/magento/app-builder-samples){target="_blank"}
