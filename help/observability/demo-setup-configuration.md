---
title: Observability demo setup and details
description: Learn how Adobe Commerce uses observability with App Builder, webhooks, and OpenTelemetry for better insights.
feature: Observability, Extensibility, App Builder
topic: Architecture, Performance
role: Architect, Developer
level: Intermediate
doc-type: Technical Video
duration: 372
last-substantial-update: 2025-12-11
jira: KT-19992
---

# Setup and Configuration

Learn how this demo was created to demonstrate Adobe Commerce and App builder observability, and to track and understand system behavior. This video shows how an App Builder project was set up with webhooks and events to manage shipping options and order validation. The example includes adding shipping carriers based on country and using random checks to show success or failure in orders. Topics such as signals, logs, and traces and how they are exported to tools like Grafana and New Relic for monitoring. The goal is to help developers see how different parts of the system work together and how observability improves troubleshooting and performance.

## Who is this video for?

* Adobe Commerce Developer
* Solutions Architect
* Technical Product Manager

## Video content

* Set up App Builder with webhooks and events for observability
* Export logs and traces to Grafana and New Relic for monitoring
* Showcase order validation and shipping logic with OpenTelemetry

>[!VIDEO](https://video.tv.adobe.com/v/3477607?learn=on&enablevpops)

{{$include /help/_includes/app-builder-observability-related-links.md}}

## Related documentation

* [Observability overview](https://developer.adobe.com/commerce/extensibility/observability/){target="_blank"}