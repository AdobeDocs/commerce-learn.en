---
user-guide-title: An Introduction using GraphQL for Adobe Commerce and Magento Open Source
user-guide-description: Learn about GraphQL to use with Adobe Commerce and Magento Open Source. 
breadcrumb-title: GraphQL Introduction
---

# Introduction to GraphQL {#graphql-rest}

+ [Get started with GraphQL](./getting-started-graphql.md)
+ [GraphQL basics](./intro-graphql.md)
+ [Queries](./graphql-queries.md)
+ [Mutations](./graphql-mutations.md)
+ [Schema](./graphql-schema.md)
+ [Return to videos, courses and tutorials](https://experienceleague.adobe.com/docs/commerce-learn/tutorials/overview.html)
